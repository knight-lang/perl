...
# All overload functions for `Ast`s just default to `run`ing the Ast. (Note that Perl converts the
# return value of `run` for us.)
use overload
	'0+'   => 'run',
	'""'   => 'run',
	'bool' => 'run',
...
# Dump returns a string debugging representation of the Ast.
sub dump {
	my ($this) = @_;
