...
sub newint :prototype($) { {kind => INT_TYPE, data => int(shift) } }
sub newstr :prototype(_) { {kind => STR_TYPE, data => shift } }
sub newlist :prototype(@) { {kind => LIST_TYPE, data => [@_] } }
sub newbool :prototype(_) { shift ? $TRUE : $FALSE }
sub lookup :prototype($) { $VARS{shift()} ||= {kind => VAR_TYPE} }
...
sub tolist :prototype($) {
	...
	return map {newstr} split //, data if STR_TYPE == kind;
...
sub parse :prototype(_) {
	...
	s/^[a-z_][a-z0-9_]*// and return lookup $&;
	...
	{ kind => AST_TYPE, func => $func, args => \@args }
}
